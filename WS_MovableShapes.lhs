...
>    move (Vec vx vy) (XY px py) = (XY (px + vx) (py + vy))

> instance Movable Figure where
>    move vec (Circle p r) = Circle (move vec p) r
>    move vec (Line pa pe) = Line (move vec pa) (move vec pe)

> instance Movable TextBox where
>    move vec (Text p s) = Text (move vec p) s
